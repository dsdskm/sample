...
fun solution(S: String, P: IntArray, Q: IntArray): IntArray {
    ...
    val res = IntArray(P.size)
    for (index in P.indices) {
        val pIndex = P[index]
        val qIndex = Q[index] + 1
        val str = S.substring(pIndex, qIndex)
        res[index] = when {
            str.contains('A') -> 1
            str.contains('C') -> 2
            str.contains('G') -> 3
...
fun solution2(S: String, P: IntArray, Q: IntArray): IntArray {
    // write your code in Kotlin
    ...
    for (index in P.indices) {
        ...
        for (sIndex in pIndex until qIndex) {
            //print(S[sIndex])
            val impact = when (S[sIndex]) {
                'A' -> 1
                'C' -> 2
                'G' -> 3
                'T' -> 4
