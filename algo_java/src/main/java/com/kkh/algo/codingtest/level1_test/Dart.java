...
public class Dart {
    ...
    public int solution(String dartResult) {
        ...
        /*
        0. 기회는 3번
        1. 문자열을 순환한다. 숫자인 경우 숫자 문자열에 계속 더한다
        2. 숫자가 아닌 경우가 나왔을때 이전 숫자 문자열을 숫자로 변환하여 문자열에 맞게 계산한다
        3. 계산된 값을 배열에 계속 넣는다
        4. *옵션이 나오는 경우 이전 배열을 순회하여 재계산후 집어 넢는다
        1S2D*3T
         */
...
//            System.out.println();
            if (c != 'S' && c != 'D' && c != 'T' && c != '#' && c != '*') {
                ...
                if (c == '#') {
                    sumArr[sumArrIndex - 1] *= -1;
                } else if (c == '*') {
//                    System.out.println("sumArr[" + (sumArrIndex - 1) + "] " + sumArr[sumArrIndex - 1]);
//                    for(int k=0;k<sumArrIndex;k++){
//                        sumArr[k] *= 2;
//                    }
                    if (sumArrIndex - 2 >= 0) {
                        sumArr[sumArrIndex - 2] *= 2;
                    }
                    if (sumArrIndex - 1 >= 0) {
                        sumArr[sumArrIndex - 1] *= 2;
                    }
                } else {
                    int num = Integer.parseInt(numSb.toString());
                    numSb = new StringBuilder();
                    if (c == 'S') {
                        sumArr[sumArrIndex++] = (int) Math.pow(num, 1);
                    } else if (c == 'D') {
                        sumArr[sumArrIndex++] = (int) Math.pow(num, 2);
                    } else if (c == 'T') {
                        sumArr[sumArrIndex++] = (int) Math.pow(num, 3);
                    }
                }
