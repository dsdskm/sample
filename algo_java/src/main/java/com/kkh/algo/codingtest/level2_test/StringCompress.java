package com.kkh.algo.codingtest.level2_test;

public class StringCompress {
    public static void main(String args[]) {
        StringCompress s = new StringCompress();
        System.out.println(s.solution("abcabcdede")); //7
//        System.out.println(s.solution("ababcdcdababcdcd")); //7

        /*
        aabbaccc -> 2a2ba3c
        ababcdcdababcdcd -> 2ab2cd2ab2cd (2개 단위)
        ababcdcdababcdcd -> 2ababcdcd (8개 단위) -> 가장짧은 케이스

        문자열 s를 1개 이상 단위로 자를때 가장 짧은 경우 문자열의 길이를 리턴

        1개 단위부터 늘려가며 반복한다
        문자열을 반복하되 shift 하면서 반복한다
        */
    }
