...
const CarouselButton = styled.div<{
	...
	position: absolute;
	font-size: 24px;
	top: calc(${({direction,position})=> direction==="row" ? "50% - 25px" : position==="left" ? "0" : "100% - 50px"});
...
const Carousel = (
	{ children: propsChildren, loop, direction = 'row',
		autoLoop, autoTime = 3000, transitionTime = 500 }: CarouselProps) => {
	...
	return <CarouselContainer direction={direction}>
		...
		{
			children.map((child, index) => <CarouselItem direction={direction}
				transitionTime={transitionTime} offset={idx} key={index}>
				{child}
			</CarouselItem>)
		}
		<CarouselButton
